...
export type TelegramErrorClass = "transient" | "permanent";
...
export function classifyTelegramFailure(
  ...
  if (status === 401) {
    return { errorClass: "permanent", parseError: false, reason: "unauthorized" };
  }
  if (status === 403) {
    return { errorClass: "permanent", parseError: false, reason: "forbidden" };
  }
  if (status === 400 && text.includes("chat not found")) {
    return { errorClass: "permanent", parseError: false, reason: "chat_not_found" };
  }
