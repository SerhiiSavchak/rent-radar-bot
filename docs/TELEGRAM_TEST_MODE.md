...
- Telegram outbox: `pending` → `sending` → `sent` only after the Bot API confirms success. Transient failures (`network`, timeout, `429`, `5xx`) stay retryable after `next_attempt_at` (2 minutes, doubling, cap 6 hours; `retry_after` can push that later). A `429` longer than 3 seconds is not retried inside the same request. `TELEGRAM_DRY_RUN=true` reports what would be sent and does not mark outbox, seen, canary, or admin alerts delivered. Item-specific non-parse `400` is not retried. `401`, `403`, and chat-not-found pause the Telegram channel in `schema_meta` (10 minutes, doubling, cap 6 hours), keep rows queued, and resume with one probe after the pause. `sending` rows recover to `pending` on reopen. Undelivered rows are not deleted by age. A crash after Telegram accepts a message and before `sent` is committed can duplicate that message; delivery prefers no loss over exactly-once. An HTML parse error is sent once more as plain text.
