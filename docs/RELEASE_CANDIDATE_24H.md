...
| **LUN** | Bare flats + houses Lviv URLs | Two category GETs | Site default only — **no explicit sort param** | **No pagination** (first response only) | Acquired cap 120/cat | End of two GETs | Silent baseline | No watermark/catch-up — downtime backlog beyond page-1 inventory **not** walked | VE: RSC + 0 cards; PF: marker/parse | Cap → degraded | **PARTIAL** — first-page-only under-covers deep backlog |
| **DIM.RIA** | Newest search `sort=created_at` + bounded details | Apt/house caps | UI “Спочатку нові” mapping in code | Search page 0 only + detail cap | Detail caps / poll budget | Cap / deferred / fail | Known-id retention | Retained ids skip re-detail | VE vs PF distinguished; **historical Sep 25 PF cause unresolved** | Truncated when deferred/fail | **PARTIAL** — improved diagnostics; history **not** closed by current OK requests |
...
Time-based stop stays off while `olx_browser_radius_sort_status=blocked`.
