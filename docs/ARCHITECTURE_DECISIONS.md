...
Status: **final for schema 8**.

SQLite stores current operations, not a historical archive. Schema 4 is the previous production shape (through `cross_source_identities`). Schema 5 adds `source_health` and the indexes used by retention. Schema 6 adds `external_seller_verifications` for linked RIELTOR seller checks. Schema 7 adds outbox retry scheduling (`next_attempt_at`, `error_class`) and `source_admin_alerts`. Schema 8 adds `seller_verification_holds` for a short linked-seller retry and rebuilds `source_health` only to allow the `coverage_degraded` status, copying every existing row. Migration is incremental: it does not delete baselines, seen listings, the outbox, seller-verification cache, or cross-source identities. Expired verification rows are deleted by the same 24-hour cleanup. A hold whose `release_at` is more than 24 hours in the past is deleted by that cleanup; an open hold inside its 20-minute window is left for the poller. Alert rows are current incident state, not a history log.

`source_health` has one row per source: `ok`, `valid_empty`, `parser_failure`, `http_error`, `rate_limited`, `transport_failure`, `browser_failure`, `coverage_degraded`, `disabled`. Healthy `ok` / `valid_empty` reset `consecutive_failures` and set `last_success_at`. Failure statuses, including `coverage_degraded`, increment the streak and leave `last_success_at` in place. `valid_empty` is stored only when the scan itself is complete. `disabled` does not change the streak. `parser_failure` is never stored as `valid_empty`, and only when an adapter returns that structured result. HTTP 429 is `rate_limited`. The poller's `transport_blocked` result (HTTP 403) is stored as `transport_failure`. A thrown network or adapter exception is also `transport_failure`. A thrown OLX Playwright acquisition is `browser_failure`. Each poll writes `disabled` for domria, lun, rieltor, and olx when that source's flag is off, including when `createCollectionAdapters` omits the adapter. OLX is one row: it is enabled when either `ENABLE_OLX` or `ENABLE_OLX_BROWSER` is on. Error text is truncated and redacted. Successful HTML is not stored.
...
Status: **final** for the poll sample. A global prefix of 10 discarded cards that the same response had already returned. DIM.RIA, LUN, and OLX now keep the rental cards from that already-acquired response, split by apartment and house, with a safety cap of 120 cards per category. Live first responses on 2026-09-22 were LUN 24 cards per category and OLX 51 apartments plus 36 houses, so the cap does not hide a normal first page. If that local cap does drop cards, the kept cards are still delivered and the source is `coverage_degraded`. That poll does not move `source_baselines.last_success_at`.

RIELTOR's public catalog default is advertising activity, not publication time. The public query `sort=bycreated` returned newest-first pages on 2026-09-22: page 1 timestamps descended, and page 2 was older than page 1, for both flats and houses, across three captures. The poll uses that order. A brand-new installation with no RIELTOR baseline fetches page 1 and records that newest publication as the committed boundary. An existing baseline without that boundary is a catch-up back to the persisted monitoring start, with a 30-minute overlap, not a new page-1 watermark. Each poll still reads page 1. A backlog longer than the 3-page request budget continues from a stored cursor on the next poll. The committed boundary advances only when that walk reaches the previous target. A truncated walk is `coverage_degraded`: the cards already fetched are still delivered, and repeated degradation uses the existing admin alert. A card with no publication time does not count as crossing the boundary.
...
## ADR: hidden-realtor profile evidence and client Telegram cards

Status: **final** for the client seller-profile policy. Three distinct addresses under one DIM.RIA `userId`, including addresses remembered in `seller_profile_cache`, are `profile_likely_intermediary`. That verdict is evidence, not confirmed intermediary proof. One or two addresses stay `unknown` and are sent. OLX does not reject on address count alone. A public profile linked from the listing page (`/uk/list/user/{slug}/`) is `profile_likely_intermediary` only when the acquired pages contain at least 10 real-estate ads. Live checks on 2026-09-24: Надія had 2 real-estate ads out of 6 on 1 page and stays sendable; Наталя Ткачук had 13 real-estate ads across 2 pages and is rejected. Two pages with a single real-estate ad stay sendable. A seller past the per-cycle probe budget is deferred, not sent, and remains eligible on a later poll. A likely cache lasts 12 hours. A successful unknown cache lasts 45 minutes. A failed read stays `unknown`, is sent, and is retried after 1 hour. The client default is `SELLER_PROFILE_LIKELY_POLICY=reject`. `SELLER_PROFILE_LIKELY_POLICY=send` still classifies and caches, and keeps the listing. A platform-confirmed owner is sent even with three or more addresses and even when the policy is reject. A new-account rule runs only when a source supplies `accountCreatedAt`, with a 7-day window, and defaults to `SELLER_PROFILE_NEW_ACCOUNT_POLICY=reject`. No creation date means no age-based rejection. OLX `isBusiness` alone stays a weak signal and does not reject. A DIM.RIA `agency_id` above zero, a RIELTOR realtor or agency role, and an OLX `company_name` stay strong confirmed-intermediary rejections. LUN and RIELTOR cards do not expose a stable private seller id, so they are not clustered. An unreadable profile is `unknown`, never a confirmed owner. The poll waits only the remainder of 10 minutes after a cycle starts, and a slow cycle starts the next one immediately without overlap. Telegram shows the price currency parsed from the source display, not an FX conversion. DIM.RIA `price` plus `currency_type` is the displayed amount; `priceArr` is the converted table and is not shown. LUN uses `price` and `currency`. RIELTOR uses the visible card price ahead of JSON-LD. OLX catalog `regularPrice` is converted UAH; the listing page `displayValue` is what Telegram shows, and only for up to 8 cards about to be sent. The catalog amount stays on `price` for dedupe.
