...
Status: **proposed** until the next review. Catalog cards do not expose account age on DIM.RIA, LUN, RIELTOR, or OLX, so no age cutoff runs in production. A new-account rule exists only for a timestamp a source actually supplies, and the conservative window is 7 days. OLX `isBusiness` alone stays a weak signal and does not reject. A DIM.RIA `agency_id` above zero is an agency link and is rejected unless the listing is a platform-confirmed owner. OLX and DIM.RIA seller ids seen on two different addresses, including addresses remembered in `seller_profile_cache`, are classified as `profile_likely_intermediary`. That verdict is evidence, not confirmed intermediary proof. Delivery defaults to `SELLER_PROFILE_LIKELY_POLICY=send` and `SELLER_PROFILE_NEW_ACCOUNT_POLICY=send`, so likely and high-risk profiles stay deliverable while the cache still accumulates addresses for soak statistics. Setting either env to `reject` drops that verdict without another code change. LUN and RIELTOR cards do not expose a stable private seller id, so they are not clustered. An unreadable profile is `unknown`, never a confirmed owner. The poll waits only the remainder of 10 minutes after a cycle starts, and a slow cycle starts the next one immediately without overlap. Telegram shows the price currency parsed from the source display, not an FX conversion. DIM.RIA `price` plus `currency_type` is the displayed amount; `priceArr` is the converted table and is not shown. LUN uses `price` and `currency`. RIELTOR uses the visible card price ahead of JSON-LD. OLX catalog `regularPrice` is converted UAH; the listing page `displayValue` is what Telegram shows, and only for up to 8 cards about to be sent. The catalog amount stays on `price` for dedupe.
